...
const AdminAnalytics = () => {
  ...
  const processReportsData = (reports: Report[]) => {
    ...
    const filteredReports = reports.filter((report) => {
      ...
      switch (timeRange) {
        ...
        case "year":
          return (
            now.getTime() - reportDate.getTime() <= 365 * 24 * 60 * 60 * 1000
          );
        default:
          return true;
      }
    ...
    const reportsByArea = Array.from(areaMap.entries())
      .map(([area, count]) => ({
        area,
        count,
      }))
  ...
  const getStatusPercentage = (status: ReportStatus) => {
    ...
    switch (status) {
      ...
      case "pending":
        return Math.round(
          (analyticsData.pendingReports / analyticsData.totalReports) * 100
        );
      case "in-progress":
        return Math.round(
          (analyticsData.inProgressReports / analyticsData.totalReports) * 100
        );
      default:
        return 0;
    }
    ...
    <Layout requireAuth allowedRoles={["admin"]}>
      <div className="container py-8">
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-3">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-green-800">
                {isLoading ? (
                  <span className="inline-flex items-center">
                    <span
                      className="animate-bounce mx-0.5 w-2 h-2 rounded-full bg-current"
          ...
          <Card
            className="bg-gradient-to-br from-blue-50 to-blue-100 animate-fade-in"
            style={{ animationDelay: "150ms" }}
          >
            <CardHeader className="pb-2">
          ...
          <Card
            className="bg-gradient-to-br from-amber-50 to-amber-100 animate-fade-in"
            style={{ animationDelay: "300ms" }}
          >
            <CardHeader className="pb-2">
        ...
        <div className="grid gap-4 mt-4 grid-cols-1 lg:grid-cols-2">
          <Card
            className="col-span-1 lg:col-span-2 animate-fade-in"
